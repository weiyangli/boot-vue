...
public class NoteBook {
    ...
    * 两者都是合并分支的命令。区别在于 git rebase 合并分支时，会将
    * 合并过来的分支的提交记录也同步到当前分支，如果存在冲突，解决冲突后
    * 通过 git rebase --continue 命令继续操作，直到全部冲突解决，不会产生新的
    * 提交，会将当前分支基于拉取分支的提交记录放置到拉取过来分支的前面。
    * git merge 不会将合并过来的分支的提交记录同步过来，不过会产生一次新的提交记录，
    * 两个分支会产生一个新的链接点。
    * */
